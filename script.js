...
restartBtn.addEventListener('click', showPasswordPopup);
confirmPasswordBtn.addEventListener('click', validatePassword);
cancelBtn.addEventListener('click', hidePasswordPopup);
closePopupBtn.addEventListener('click', hidePasswordPopup);
...
async function loadQuizData() {
    try {
        ...
        const response = await fetch('assets/gabarito.xlsx');
        if (!response.ok) {
...
function displayQuestion() {
    ...
    vehicles.forEach((vehicle, index) => {
        ...
        checkbox.addEventListener('change', handleVehicleSelection);
        
        // Event listener para clicar na div
...
function calculateResults() {
    ...
    const results = quizData.map((question, index) => {
        ...
        // Pontuação: +1 para cada seleção correta, -0.5 para cada seleção incorreta
        const questionScore = correctSelections.length - (incorrectSelections.length * 0.5);
        const maxQuestionScore = correctAnswer.length;
        
        correctAnswers += Math.max(0, questionScore);
    ...
    return {
        ...
        totalPossible: Math.round(totalPossibleCorrect)
    };
}
...
// Event listener para ESC no popup
document.addEventListener('keydown', function(event) {
    if (event.key === 'Escape' && passwordPopup.classList.contains('active')) {
        hidePasswordPopup();
    }
});
